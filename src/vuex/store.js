...
const searchState = {
  ...
  actions: {
    LOAD_SEARCH_RESULTS({ commit, rootState }) {
    ...
    LOAD_MANGA_GENRE_LIST({ commit, rootState }) {
...
const mangaState = {
  ...
  actions: {
    LOAD_MANGA_INFO({ commit, rootState }) {
    ...
    LOAD_MANGA_CHAPTER({ commit, rootState }) {
  ...
  getters: {
    currentPage: (state, getters) => id => state.pages.find(page => page.pageId === id),
  },
};
