<template>
  <div>
    <div
      class="columns"
      v-on:keyup.left="prevPageLink"
      v-on:keyup.right="nextPageLink"
      tabindex="-1"
    >
      <div
        class="column is-12-mobile is-6-tablet is-offset-3-tablet"
        style="position: fixed; bottom: 10px; z-index: 99; left: -4px;"
      >
        <div class="card">
          <div class="card-content">
            <nav class="is-hidden-tablet pagination is-centered">
              ...
              <div
                ...
                v-if="currentChapter && currentChapter.pages.length"
              >
                Ch. {{ params.chapterId }} /
                {{ params.page }}
                of {{ totalPage }}
...
export default {
  ...
  computed: {
    ...
    currentPage() {
      ...
      return this.$store.state.manga.chapter.pages.find(page => page.pageId === parseInt(pageId, 10));
    },

    nextPage() {
      if (this.params.page < this.currentChapter.pages.length) {
        return parseInt(this.params.page, 10) + 1;
      }
    ...
    prevPage() {
      if (parseInt(this.params.page, 10) > 1) {
        return parseInt(this.params.page, 10) - 1;
      }
